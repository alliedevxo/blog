...
- Type around domain resources, not around the current feature UI.
- Avoid copy-paste nested object types and "everything optional" types.
- Break larger payloads into reusable chunks (`Ticket`, `UserReference`, `Status`, etc.).
- For mutations, pass the smallest specific type needed.
...
### First attempt: one big Ticket type
...
This is the first mistake I often see. It’s technically correct, but not maintainable.

When another type needs a person shape (`id`, `name`), there is nothing reusable here.
Even in this example, `reporter` and `assignee` are duplicates.
...
- Just because we don’t care about data today does not mean the type disappears from the API.
- We still have duplication.
- `reporter` and `assignee` are now different shapes even though they represent the same backend concept.

These are the two most common typing antipatterns I see:

1. A giant non-reusable interface
2. A feature-coupled trimmed interface
...
Before we move on, these should live in sensible locations. You do not want every type in `ticket.d.ts` if unrelated features need `Person`/`User`.

`Person` is much more generic than `Ticket`, so `Ticket` can import `Person` from wherever it makes sense.

Now that we have base types, `Tickets` becomes easy:
...
For `PATCH`, separation really pays off.

When we patch a resource, we usually update one part of it, not the full payload. So pass the smallest specific type.
...
This can update any part of the ticket and can cause unintended side effects.
...
This isolates the update to status and avoids mutating the ticket object.

You can imagine other functions for updating the assignee, adding/removing/updating notes, etc.

---

## Future feature that reuses these types
...
The person shape should be reusable here. The server will likely return the same core user identity fields plus additional profile fields.
...
Most important pattern to avoid: separating these types entirely.
...
This is the lazy way of typing. It lets you define one type, but makes downstream code harder to reason about.
...
To be clear: both options above are antipatterns. I include them because they are common in real codebases.
...
I have a love/hate relationship with TypeScript utility types, but I like this use of `Pick`.
...
One con is repeating field names in `Pick`. The upside is compile-time failure if those fields change.
...
Clear and concise: now these types support a ticket dashboard, user avatars, and a profile page.

---

## Wrap up

We looked at common antipatterns and then moved to resource-oriented alternatives.

When typing code, think in terms of resources and what those types truly represent. This avoids feature-coupled types and scales better across the app.
